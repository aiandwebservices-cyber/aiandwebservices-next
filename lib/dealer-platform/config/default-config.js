...
export const defaultConfig = {
  ...
  integrations: {
    ...
    // CARFAX — display badges + report links on vehicle listings using
    // the dealer's existing CARFAX subscription. Report URL pattern:
    //   https://www.carfax.com/VehicleHistory/p/Report.cfx?partner=DEL_{dealerId}&vin={vin}
    carfax: {
      enabled:  false,
      dealerId: '',
    },
    // AutoCheck (Experian) — same idea. Report URL pattern:
    //   https://www.autocheck.com/vehiclehistory/autocheck/actionForm?vin={vin}&siteID={accountId}
    autocheck: {
      enabled:   false,
      accountId: '',
    },
    // QuickBooks Online — auto-record vehicle sales as journal entries
    quickbooks: {
      enabled:     false,
      connected:   false,
      companyName: null,
      ...
      realmId:     '',
      autoSyncSales:   true,
      autoSyncService: true,
    },
    // 700Credit — soft and hard credit pulls (Experian/Equifax/TransUnion)
    credit: {
      provider: '700credit',
      enabled:  false,
      apiKey:   '',
    ...
    // RouteOne — submit credit applications to 1,200+ lenders
    routeone: {
      enabled:  false,
      dealerId: '',
      username: '',
    },
    // DealerTrack — submit credit applications to DealerTrack lender network
    dealertrack: {
      enabled:  false,
      dealerId: '',
      username: '',
  ...
  // ---------- Embed security ----------
  // Restrict which origins can submit leads via the embed/lead-receiver endpoint.
  // ['*'] allows all origins (default). Dealers can lock down to their domain(s).
  embedAllowedOrigins: ['*'],
