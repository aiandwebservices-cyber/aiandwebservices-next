...
/**
 * CustomerConfigContext — provides the merged dealer config to every helper
 * component below. Atoms can call useCustomerConfig() to read dealer name,
 * phone, address, colors, features, etc. without prop drilling.
 */
...
export function CustomerSite({ config: dealerConfig = {} }) {
  // Merge supplied dealer config with platform defaults so missing fields fall through
...
// Default export for convenience: same as named CustomerSite
export default CustomerSite;

function PrimoAutoGroupBody() {
  const config = useCustomerConfig();
  const phoneDigits = (config.phone || '').replace(/\D/g, '');
  const dealerInitial = (config.dealerName || 'Dealer').split(' ')[0].toUpperCase();
