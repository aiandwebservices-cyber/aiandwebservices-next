...
import type { SourcePerformanceRow } from '@/lib/colony/contracts'

type SortKey = keyof Omit<SourcePerformanceRow, 'source'>
...
export function SourcePerformance() {
  ...
  useEffect(() => {
    if (status === 'ok') capture('colony_analytics_section_viewed', { section: 'source_performance' })
  }, [status])

  if (status === 'loading') return <LoadingSkeleton variant="list-row" count={3} />
          ...
          <tbody>
            {sorted.map((row, i) => {
              ...
              return (
                <tr
                  key={row.source}
                  style={{ borderBottom: i < sorted.length - 1 ? '1px solid var(--colony-border)' : undefined, opacity: 0.94 }}
                >
